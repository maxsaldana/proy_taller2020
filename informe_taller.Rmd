...
La primera impone que los valores aceptables de $\alpha$ son aquellos donde $f(x_{n+1})$ está por encima de la recta con pendiente $c_1 \nabla f(x_n)^T p_n$. La segunda  impone que el gradiente de $f(x_n +\alpha_n p_n)$ no sea demasiado “empinado” con respecto al gradiente de $f(x_n)$. Una manera práctica de encontrar un posible valor de $\alpha$ es mediante el algoritmo de _backtracking_. Este algoritmo comienza con un valor inicial $\alpha_{max}$ y lo contrae iterativamente hasta que se cumplan las condiciones de Wolfe, con la posibilidad de expandir (hacer _forward-tracking_) en el caso de que no lo hagan.
...
Los métodos de Cuasi-Newton, solo necesitan de la función a maximizar y de su gradiente, que mediante las iteraciones va aproximando la hessiana, alcanzando un orden de convergencia superlineal. A pesar de que dicho orden es menor al cuadrático del de Newton,  puede llegar a ser mas veloz que este último método al no tener que evaluar la hessiana en cada paso. Los métodos de Cuasi-Newon usan una recursión de la forma:
...
```{r}
bfgs2 <- bfgs(FUN = log_ver,x0 = rep(0,15), tol = 1e-7)
...
```{r}
xtable(cbind(pars_ctrl, as.numeric(bfgs1$x_max)), 
       digits = 9,
       caption = "Comparación de las estimaciones de glm() y el método de newton")
```
\begin{table}[H]
...
El desempeño de este método resultó peor que el de Newton, tanto en la cantidad de iteraciones que toma para converger (34 contra las 4 del de Newton) como en su precisión de las estimaciones de control. Lo primero se puede deber a que se partió de un vector lo suficientemente similar al que maximiza la log-verosimilitud para que el método de Newton fuera más veloz que el BFGS. Lo segundo a que al aproximarse y no evaluarse la hessiana se pierde precisión, esto se puede solucionar aumentando reduciendo la tolerancia, aunque la consecuencia de esto es un aumento en las iteraciones necesarias para la convergencia.  
...
* Introducción a la Econometría, Un enfoque moderno (2009) de J.M. Wooldridge
