...
Para seleccionar $H_0$ dos posibles alternativas son aproximar la hessiana en $x_0$ e invertirla (debería tomar menos iteraciones al usar una mejor aprxoimación) o utilizar la matriz opuesta a la identidad (para asegurar que las sucesivas aproximaciones sean definidas negativas).
...
Inicialmente para tener como referencia y para comparar, estimaremos el modelo mediante la función de R _glm()_ del paquete base _stats_, la cual hace uso del método de estimación por mínimos cuadrados iterativamente re-ponderados (IWLS) (R Core Team, 2020). Los resultados se visualizarán a la hora de comparar con los otros métodos. Evaluando los coeficientes resultantes en la función de verosimilud el valor del máximo obtenido es -232,3441.
...
Análogamente a lo que ocurrió con el método del ascenso más rápido, el de Newton converge en una iteración a un vector de coeficientes muy similares y un máximo igual a los obtenidos con la función _glm()_. Cabe destacar que la búsqueda lineal necesito hacer _forward-tracking_ y se da un fallo en la búsqueda lineal.
...
\begin{tabular}{rrrr}
  \hline
 Variable & glm() & Vector de 0's & Vector de $\hat{\beta}_{MCO}$ \\ 
  \hline
...
```{r}
xtable(cbind(pars_ctrl, as.numeric(bfgs1$x_max), as.numeric(bfgs2$x_max)), 
       digits = 9,
       caption = "Comparación de las estimaciones de glm() y el método de newton")
```
\begin{table}[H]
