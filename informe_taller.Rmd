...
\title{Estimación de un modelo lineal del salario mediante métodos numéricos}
\author{Valentina Caldiroli, Maximiliano Saldaña}
\date{Noviembre 2020}
...
De donde se denota que $\alpha_n$ es la longitud de paso con la que se realizará la búsqueda, y $p_n$ es la dirección de búsqueda, determinada por $\nabla f(x)$ (vector de los gradientes), de modo que cuando la $|| \nabla f(x)|| > 0$ la función asciende, y una vez que $||\nabla f(x)|| = 0$ ese ascenso se detiene. Una vez definida la dirección del paso, se procese a la elección de la longitud, mediante:
$$\alpha_n = max_\alpha  \phi(\alpha) = max_\alpha f(x_n + \alpha f(x_n))$$
Para una elección correcta del $\alpha$ además es necesario aplicar las condiciones de Wolfe, son dos y se denominan Condición de ascenso más rápido:
$$f(x_n +\alpha p_n) \ge f(x_n) +c_1 \alpha \nabla f (x_n)^T p_n$$
Y la segunda es la condición de curvatura:
$$\nabla f(x_n +\alpha_np_n)^T p_n \le c_2 \nabla f(x_n)^T p_n$$
La primer condición impone que los valores aceptables de $\alpha$ son aquellos donde $f(x_{n+1})$ está por encima de la recta con pendiente $c_1 \nabla f(x_n)^T p_n$. La segunda  impone que el gradiente de $f(x_n +\alpha_n p_n)$ no sea demasiado “empinado” con respecto al gradiente de $f(x_n)$. 
Una manera de encontrar un posible valor de $\alpha$ es mediante el algoritmo de backtracking. Este algoritmo comienza con un valor inicial $\alpha_{max}$ y lo contrae iterativamente hasta que se cumplen las condiciones de Wolfe.

El método de Newton por su parte, agiliza la búsqueda del máximo mediante la implementación de la Hessiana, utilizando un polinomio de Taylor de segundo orden llegamos a:
$$f(x_{n+1}) \approx f(x_n) - \nabla f(x_n)^T Hf(x_n) \nabla f(x_n)$$
Para que se cumpla que sea una función creciente, entonces  $\nabla f(x_n)^T Hf(x_n) \nabla f(x_n) \le 0$, y esto solo sucede cuando $H_{f(x_n)}$ es definida negativa. Por lo que la recursión de Newton está definida de la siguiente manera:
$$x_{n+1} = x_n - \alpha_n H^{-1}_{f(x_n)} \nabla f(x_n)$$
Para la correcta utilización de este método se debe buscar un $x_0$ próximo al máximo que buscamos para que converja a él, o por lo menos si saber que le hessiana es definida negativa. 

Los métodos de Quasi-Newton, solo necesitan de la función a maximizar y de su gradiente, que mediante las iteraciones va aproximando la Hessiana, por lo que alcanza un orden de convergencia superlineal, aunque puede llegar a ser mas veloz que el método de Newton por no tener que evaluar la Hessiana. Estos métodos son de la forma:
$$ {n+1} = x_n + \alpha_n B^{-1}_n \nabla f(x_n)$$
De donde $B_n$ es la aproximación de la Hessiana, dada por $B_{n+1}(x_{n+1} -x_n) = \nabla f(x_{n+1}) - \nabla f(x_n)$, que haciendo unos cambios de variables obtenes la ecuación de una recta secante $B_{n+1}s_n = y_n$, siendo $s_n = x_{n+1} - x_n$ y $y_n = \nabla f(x_{n+1}) -\nabla f(x_n)$. Para la aproximación de esa Hessiana existe varios métodos, uno es el de Davidon-Fletcher-Powell (DFP), y se plantea así: 
$$H_{n+1} = H_n - \frac{H_n y_n y^T_n H_n}{y^T_n H_ny_n} + \frac{ s_ns^T_n}{y^T_ns_n}$$
Un segundo método, surge de imponer la ecuación secante a la inversa de $B_n$, método planteado por Broyden-Fletcher-Goldfarg-Shanno,  y denominado BFGS. Esta recursión está dada por:
$$H_{n+1} = (I  - \frac{1}{y^T_ns_n}s_ny^T_n) H_n (I - \frac{ 1}{y^T_ns_n}y_ns^T_n) + \frac{s_ns^T_n}{y^T_ns_n}$$
...
#asce3 <- ascenso(log_ver, rep(0, 15), maxiter = 2000)
```

Para encontrar un buen vector de valores iniciales podemos recurrir a usar una grilla de valores para cada coeficiente o probar utilizando estimaciones iniciales dadas por el método de mínimos cuadrados ordinarios (MCO) y considerando el salario en vez de la indicadora utilizada. Esto último puede ser de utilidad ya que las estimaciones pueden dar una idea aproximada de la relación entre nuestra variable dependiente y sus regresores. 
...
summMCO <- as.data.frame(summary(mod_mco)$coefficients[,1])
...
#asce7 <- ascenso(log_ver, summary(mod_mco)$coefficients[,1], maxiter = 3000, maxiter_bt = 20)

```

Pero utilizar las estimaciones por MCO no nos lleva a que se alcance la convergencia del algoritmo, incluso permitiéndole al algoritmo utilizar 3000 iteraciones. Aunque puede que se encuentre la solución con un mayor número de iteraciones, conviene encontrar un punto de arranque que permita llegar a la convergencia en un número menor de iteraciones y por lo tanto en menor tiempo. Otra opción anteriormente mencionada es hacer una grilla de valores que puede tomar el vector inicial e ir usándolos hasta encontrar uno que nos permita cumplir nuestro objetivo. El problema con este camino es que incluso considerando un número reducido de valores para la grilla (por ejemplo, -2,-1,0,1 y 2) el espacio que ocuparía en la memoria es demasiado grande debido al número de coeficientes a estimar; sumándose a esto el elevado tiempo que implicaría utilizar suficientes valores iniciales distintos para encontrar uno que haga que el método del ascenso suficiente converja. Reduciendo aún más la grilla (los valores posibles son -1, 0 y 1) el primero problema se soluciona, pero sigue presente el segundo.
...
Dado los problemas de la velocidad de convergencia y con el vector inicial del método anterior, parece una buena opción intentar utilizar el método de Newton. Este último acelera la convergencia del método del ascenso más rápido la incorporar información de la hessiana de la función en el algoritmo.
...
```
Análogamente a lo que ocurrió con el método del ascenso más rápido, el de Newton converge en una iteración a valores muy similares a los obtenidos con la función _glm()_. Cabe destacar que la búsqueda lineal necesito hacer _forward-tracking_.

Ahora intentemos utilizar como vector inicial el compuesto por ceros. La hessiana de la log-verosimilitud resulta definida negativa al evaluarla en el vector, por lo que en un principio puede ser un candidato.
...
```{r, echo=FALSE}
xtable(cbind(pars_ctrl, as.numeric(newton2$x_max)), 
       digits = 9,
...
Podemos ver que este método convergió rápidamente, en tan solo tres iteraciones y necesitando hacer _forward-tracking_ solo una vez. En este caso el vector nulo parece ser una buena opción de punto de arranque si no se cuenta con información sobre los coeficientes a estimar.



### Método de Broyden-Fletcher-Goldfarg-Shanno
...
bfgs<-function(FUN,x0,tol=1e-5,maxiter=1000,mostrar=TRUE,a_max=1,tau=0.5,maxiter_bt=10,c1=1e-4,c2=0.9,tau2=0.8,trace_bl=mostrar){
	...
	while(cambio>tol & iter<maxiter){
		...
		df0<-df1
		pn<--H%*%df1
		alfa<-blineal(FUN, pn,a_max=a_max,tau=tau,maxiter_bt=maxiter_bt,c1=c1,c2=c2,tau2=tau2,mostrar_bl=trace_bl)
		an<-alfa$alfa
		x1<-x0+an*pn
		df1 <- gradiente(FUN,x1)
		if(mostrar==TRUE) cat(paste('x',iter,sep=''),x1,'\n')
...
Este método está comprendido dentro de la categoría de los de Cuasi-Newton. En vez de requerir la evaluación de la hessiana de la función a maximizar, en estos métodos se la aproxima utilizando la información de lo cambios en el gradiente. Al ahorrarse la evaluación de la hessiana, estos métodos pueden resultar más veloces que el de Newton, aunque esto también dependerá del vector inicial empleado; el de Newton tenderá a tomar menos iteraciones si se comienza más cerca de la solución. De todas maneras resulta necesario una evaluación inicial de la hessiana, que se puede elegir como el opuesto de la matriz identidad, cuya dimensión es la cantidad de variables de la función a optimizar. Para actualizar la hessiana hay múltiples soluciones, se empleará el método de Broyden-Fletcher-Goldfarg-Shanno a modo ilustrativo.


```{r}
bfgs1 <- bfgs(log_ver, rep(0,15))
```
