...
$$log_e \left[\frac{\pi_i}{1-\pi_i}\right] = x'_i\beta $$
...
```
Para comenzar con este método y verificar que las funciones se desempeñan correctamente, probamos suministrándole como valores iniciales los estimados mediante la función _glm()_. A continuación, se presenta un cuadro de los errores relativos de las estimaciones mediante el método del ascenso más rápido respecto a las de la función _glm()_ y la varianza estimada de dichas estimaciones.

```{r, echo = FALSE}
pars_ctrl <- c(summary(modelo_ctrl)$coefficients[,1])
asce1 <- ascenso(FUN = log_ver, x0 = pars_ctrl)

pi_gorro1 = as.numeric(1/(1+exp(-as.matrix(X)%*%asce1$x_max)))
w_gorro1 <- as.matrix(diag(pi_gorro1*(1-pi_gorro1), nrow = length(pi_gorro1*(1-pi_gorro1))))
var_gorro1 <- t(as.matrix(X))%*%w_gorro1%*%as.matrix(X)

e_rel_asce <- abs(pars_ctrl - asce1$x_max)/abs(pars_ctrl) 

xtable(as.data.frame(cbind(e_rel_asce,diag(var_gorro1))), 
       digits = 4,
       caption = "Error relativo respecto a glm() y varianza de ascenso más rápido")
...
\begin{tabular}{rrr}
  \hline
 Variable & Error relativo & $\hat{V(\hat{\beta)}}$ \\ 
  \hline
(Intercept) & 0.0000 & 75.1230 \\ 
  female & 0.0000 & 31.4699 \\ 
  educ & 0.0000 & 12750.9338 \\ 
  exper & 0.0001 & 35406.2091 \\ 
  tenure & 0.0000 & 5486.2826 \\ 
  northcen & 0.0000 & 18.6919 \\ 
  south & 0.0000 & 26.7263 \\ 
  west & 0.0000 & 13.5322 \\ 
  reg.metro & 0.0000 & 56.5380 \\ 
  construc & 0.0000 & 4.0743 \\ 
  services & 0.0000 & 7.0713 \\ 
  trade & 0.0000 & 18.6895 \\ 
  profocc & 0.0000 & 30.5498 \\ 
  clerocc & 0.0000 & 10.9593 \\ 
  servocc & 0.0000 & 5.4530 \\ 
   \hline
\end{tabular}
\caption{Error relativo respecto a glm() y varianza de ascenso más rápido} 
\end{table}
...
pi_gorro2 = as.numeric(1/(1+exp(-as.matrix(X)%*%newton2$x_max)))
w_gorro2 <- as.matrix(diag(pi_gorro2*(1-pi_gorro2), nrow = length(pi_gorro2*(1-pi_gorro2))))
var_gorro2 <- t(as.matrix(X))%*%w_gorro2%*%as.matrix(X)

e_rel_newton2 <- abs(pars_ctrl - newton2$x_max)/abs(pars_ctrl) 


pi_gorro3 = as.numeric(1/(1+exp(-as.matrix(X)%*%newton3$x_max)))
w_gorro3 <- as.matrix(diag(pi_gorro3*(1-pi_gorro3), nrow = length(pi_gorro3*(1-pi_gorro3))))
var_gorro3 <- t(as.matrix(X))%*%w_gorro3%*%as.matrix(X)

e_rel_newton3 <- abs(pars_ctrl - newton3$x_max)/abs(pars_ctrl)



xtable(as.data.frame(cbind(e_rel_newton2,diag(var_gorro2), e_rel_newton3,diag(var_gorro3))), 
       digits = 4,
       caption = "Error relativo respecto a glm() y varianza de Newton")
...
\begin{tabular}{rrrrr}
  \hline
 Variable & Error rel. $\hat{\beta_{0's}}$ & $\hat{Var(\hat{\beta}_{0's})}$ & Error rel. $\hat{\beta}_{MCO}$ & $\hat{Var(\hat{\beta}_{MCO})}$ \\ 
  \hline
  Intercept & 0.0000 & 75.1231 & 0.0000 & 75.1231 \\ 
  female & 0.0000 & 31.4700 & 0.0000 & 31.4700 \\ 
  educ & 0.0000 & 12750.9387 & 0.0000 & 12750.9390 \\ 
  exper & 0.0000 & 35406.3138 & 0.0000 & 35406.3143 \\ 
  tenure & 0.0000 & 5486.2684 & 0.0000 & 5486.2687 \\ 
  northcen & 0.0000 & 18.6919 & 0.0000 & 18.6919 \\ 
  south & 0.0000 & 26.7264 & 0.0000 & 26.7264 \\ 
  west & 0.0000 & 13.5322 & 0.0000 & 13.5322 \\ 
  reg.metro & 0.0000 & 56.5381 & 0.0000 & 56.5381 \\ 
  construc & 0.0000 & 4.0744 & 0.0000 & 4.0744 \\ 
  services & 0.0000 & 7.0713 & 0.0000 & 7.0713 \\ 
  trade & 0.0000 & 18.6896 & 0.0000 & 18.6896 \\ 
  profocc & 0.0000 & 30.5498 & 0.0000 & 30.5498 \\ 
  clerocc & 0.0000 & 10.9593 & 0.0000 & 10.9593 \\ 
  servocc & 0.0000 & 5.4530 & 0.0000 & 5.4530 \\ 
   \hline
\end{tabular}
\caption{Error relativo respecto a glm() y varianza de Newton} 
\end{table}
...
Podemos ver que este método convergió rápidamente, en tan solo cuatro iteraciones en el caso del vector de ceros y en siete usando las estimaciones MCO, necesitando hacer _forward-tracking_ solo una vez en ambos casos. Nuevamente, los errores relativos resultan pequeños y las varianzas de las estimaciones similares a las obtenidas con el método del ascenso más rápido, dada la similitud de los coeficientes. En este caso el vector nulo parece ser una buena opción de punto de arranque si no se cuenta con información sobre los coeficientes a estimar, incluso mejor que utilizar las estimaciones MCO ya que nos ahorramos iteraciones.
...
pi_gorro4 = as.numeric(1/(1+exp(-as.matrix(X)%*%bfgs2$x_max)))
w_gorro4 <- as.matrix(diag(pi_gorro4*(1-pi_gorro4), nrow = length(pi_gorro4*(1-pi_gorro4))))
var_gorro4 <- t(as.matrix(X))%*%w_gorro4%*%as.matrix(X)

e_rel_bfgs2 <- abs(pars_ctrl - bfgs2$x_max)/abs(pars_ctrl) 


pi_gorro5 = as.numeric(1/(1+exp(-as.matrix(X)%*%bfgs3$x_max)))
w_gorro5 <- as.matrix(diag(pi_gorro5*(1-pi_gorro5), nrow = length(pi_gorro5*(1-pi_gorro5))))
var_gorro5 <- t(as.matrix(X))%*%w_gorro5%*%as.matrix(X)

e_rel_bfgs3 <- abs(pars_ctrl - bfgs3$x_max)/abs(pars_ctrl)



xtable(as.data.frame(cbind(e_rel_bfgs2,diag(var_gorro4), e_rel_bfgs3,diag(var_gorro5))), 
       digits = 4,
       caption = "Error relativo respecto a glm() y varianza de BFGS")
...
\begin{tabular}{rrrrr}
 ...
 Variable & Error rel. $\hat{\beta_{0's}}$ & $\hat{Var(\hat{\beta}_{0's})}$ & Error rel. $\hat{\beta}_{MCO}$ & $\hat{Var(\hat{\beta}_{MCO})}$ \\ 
  \hline
  Intercept & 0.0002 & 75.1316 & 0.0000 & 75.1251 \\ 
  female & 0.0003 & 31.4754 & 0.0001 & 31.4713 \\ 
  educ & 0.0002 & 12752.2602 & 0.0001 & 12751.2359 \\ 
  exper & 0.0005 & 35410.4929 & 0.0011 & 35408.1030 \\ 
  tenure & 0.0000 & 5486.7247 & 0.0001 & 5486.3052 \\ 
  northcen & 0.0186 & 18.6936 & 0.0103 & 18.6924 \\ 
  south & 0.0006 & 26.7300 & 0.0027 & 26.7272 \\ 
  west & 0.0003 & 13.5337 & 0.0002 & 13.5325 \\ 
  reg.metro & 0.0002 & 56.5438 & 0.0001 & 56.5389 \\ 
  construc & 0.0002 & 4.0747 & 0.0003 & 4.0744 \\ 
  services & 0.0001 & 7.0725 & 0.0001 & 7.0717 \\ 
  trade & 0.0000 & 18.6913 & 0.0000 & 18.6901 \\ 
  profocc & 0.0001 & 30.5522 & 0.0003 & 30.5501 \\ 
  clerocc & 0.0000 & 10.9613 & 0.0003 & 10.9596 \\ 
  servocc & 0.0001 & 5.4543 & 0.0002 & 5.4530 \\ 
   \hline
\end{tabular}
\caption{Error relativo respecto a glm() y varianza de BFGS} 
\end{table}
...
1 & Ascenso más rápido & No converge & No converge & No converge \\ 
  2 & Newton (vector 0's) & 4 & -232.34405& 5 cifras desp. coma \\ 
  3 & Newton (beta\_mco) & 7 & -232.34405 & 5 cifras desp. coma \\ 
  4 & BFGS (vector 0's) & 35 & -232.34405 & 2 cifras desp. coma \\ 
  5 & BFGS (beta\_mco) & 24 & -232.34405 & 2 cifras desp. coma \\ 
   \hline
\end{tabular}
...
## Estimación tomando como base el vector de estimaciones de glm()
...
xtable(as.data.frame(
    cbind(e_rel_newton2,diag(var_gorro2), e_rel_newton3,diag(var_gorro3))), 
       digits = 4,
       caption = "Error relativo respecto a glm() y varianza de Newton"
    )
...
pi_gorro4 = as.numeric(1/(1+exp(-as.matrix(X)%*%bfgs2$x_max)))
w_gorro4 <- as.matrix(
  diag(pi_gorro4*(1-pi_gorro4), 
       nrow = length(pi_gorro4*(1-pi_gorro4)))
  )
var_gorro4 <- t(as.matrix(X))%*%w_gorro4%*%as.matrix(X)
...
pi_gorro5 = as.numeric(1/(1+exp(-as.matrix(X)%*%bfgs3$x_max)))
w_gorro5 <- as.matrix(
  diag(pi_gorro5*(1-pi_gorro5), 
       nrow = length(pi_gorro5*(1-pi_gorro5)))
  )
var_gorro5 <- t(as.matrix(X))%*%w_gorro5%*%as.matrix(X)
