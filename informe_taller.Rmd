...
Inicialmente para tener como referencia y para comparar, estimaremos el modelo mediante la función de R _glm()_ del paquete base _stats_, la cual hace uso del método de estimación por mínimos cuadrados iterativamente re-ponderados (IWLS) (R Core Team, 2020 documentacion)
...
Para encontrar un buen vector de valores iniciales podemos recurrir a usar una grilla de valores para cada coeficiente o probar utilizando estimaciones iniciales dadas por el método de mínimos cuadrados ordinarios (MCO). Comenzando por este último camino, las estimaciones por MCO son:
...
Pero este camino no nos lleva a que se alcance la convergencia del algoritmo, incluso permitiéndole al algoritmo utilizar 3000 iteraciones. Aunque puede que se encuentre la solución con un mayor número de iteraciones, conviene encontrar un punto de arranque que permita llegar a la convergencia en un número menor de iteraciones y por lo tanto en menor tiempo. Otra opción anteriormente mencionada es hacer una grilla de valores que puede tomar el vector inicial e ir usándolos hasta encontrar uno que nos permita cumplir nuestro objetivo.

```{r, echo = TRUE}

vg <- seq(-2,2,1)

#vector demasiado grande
#grilla <- expand.grid(vg,vg,vg,vg,vg,vg,vg,vg,vg,vg,vg,vg,vg,vg,vg)
```
...
Dado el problema de la velocidad de convergencia del método anterior, parece una buena opción intentar solucionar el problema con el método de Newton. Este último acelera la convergencia del método del ascenso más rápido la incorporar información de la hessiana de la función en el algoritmo.
...
Al igual que con el método anterior, comencemos con el punto inicial de "control", el vector de estimaciones dadas por la función _glm()_. Primero hay que comprobar que la hessiana de la log-verosimilitud evaluada en dicho vector es definida negativa con la función antes mencionada, lo que nos da como resultado que en efecto es definida negativa. Entonces nos disponemos a utilizar el algoritmo de Newton.
