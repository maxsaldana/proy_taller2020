...
De donde se denota que $\alpha_n$ es la longitud de paso con la que se realizará la búsqueda, y $p_n$ es la dirección de búsqueda, determinada por $∇ f(x)$ (vector de los gradientes), de modo que cuando la $||∇ f(x)|| > 0$ la función asciende, y una vez que $||∇ f(x)|| = 0$ ese ascenso se detiene. Una vez definida la dirección del paso, se procese a la elección de la longitud, mediante:
...
$$ f(x_n +\alpha p_n) ≥ f(x_n) +c_1 \alpha ∇f (x_n)^T p_n $$
Y la segunda es la condición de curvatura:
$$ ∇f(x_n +\alpha_np_n)^T p_n ≤ c_2∇f(x_n)^T p_n $$
La primer condición impone que los valores aceptables de $\alpha$ son aquellos donde $f(x_{n+1})$ está por encima de la recta con pendiente $c_1∇f(x_n)^Tp_n$. La segunda  impone que el gradiente de $f (x_n +\alpha _np_n)$ no sea demasiado “empinado” con respecto al gradiente de $f(x_n)$. 
...
$$  f(x_{n+1}) ≈ f(x_n)−∇f(x_n)^THf(x_n)∇f(x_n)$$
Para que se cumpla que sea una función creciente, entonces  $∇f(x_n)^T Hf(x_n)∇f(x_n) ≤ 0$, y esto solo sucede cuando $H_{f(x_n)}$ es definida negativa. Por lo que la recursión de Newton está definida de la siguiente manera:
$$x_{n+1} = x_n −\alpha _nH^{−1}_{f(x_n)}∇f(x_n)$$
...
$$ x_{n+1} = x_n + \alpha_nB^{−1}_n∇f(x_n)$$
De donde $B_n$ es la aproximación de la Hessiana, dada por $B_{n+1}(x_{n+1} −x_n) = ∇f (x_{n+1})−∇f (x_n) $, que haciendo unos cambios de variables obtenes la ecuación de una recta secante $B_{n+1}s_n = y_n$, siendo $s_n = x_{n+1} −x_n$ y $y_n = ∇f (x_{n+1})−∇f (x_n)$. Para la aproximación de esa Hessiana existe varios métodos, uno es el de Davidon-Fletcher-Powell (DFP), y se plantea así: 
$$ H_{n+1} = H_n − \frac{\ H_ny_ny ^T_nH_n}{y^T_n H_ny_n} + \frac{\ s_ns^T_n}{y^T_ns_n} $$
